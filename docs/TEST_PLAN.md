...
- Privacy-safe diagnostics: operation identifiers may be logged, while exception message, stack, vault fields, passphrases, keys, and decrypted attachments are absent; temporary redacted diagnostic share files should be deleted where permitted. Capability-probe failures must not reintroduce raw `Debug.WriteLine` exception messages.
