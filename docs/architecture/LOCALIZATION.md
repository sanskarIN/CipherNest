...
- Dynamic TOTP period/validity text, TOTP operation status/error messages, Unlock workflow statuses, onboarding strength labels, vault-creation statuses, and Settings security-operation statuses resolve the same reviewed catalog and use `CultureInfo.CurrentUICulture` where numeric formatting is required.
...
- Dedicated localization source tests guard TOTP, Unlock, onboarding/recovery, About security/privacy, and Settings resource catalogs, XAML usage, dynamic/status formatting, fail-safe messages, and removal of selected previous hard-coded security copy.

## Reviewed Hindi scope

The current Hindi catalog covers the resource-backed product/title/navigation controls already represented by `AppStrings` plus the security-sensitive local-only, audit-status, recovery-limitation, language-preference status messages, migrated TOTP workflow, initial Unlock workflow, initial vault-onboarding/recovery workflow, About security/privacy claims, and the migrated Settings fixed surface/security operations.
...
The resource-backed Settings surface now includes:
...
11. Keep security warnings semantically equivalent; translations must not weaken recovery, export, audit, deletion, biometric, clipboard, TOTP, funding, storage/cache, or platform-limit wording.
...
- review every translated recovery, biometric, TOTP, destructive-action, funding, storage/cache, and audit warning against the canonical English security documentation;
