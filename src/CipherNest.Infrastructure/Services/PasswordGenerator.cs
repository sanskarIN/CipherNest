...
public sealed class PasswordGenerator : IPasswordGenerator
{
    ...
    {
        if (string.IsNullOrEmpty(secret)) return new PasswordStrengthResult(0, "Empty", ["Use a long, unique passphrase or generated password."]);
        var score = 0;
        var suggestions = new List<string>();
        if (secret.Length >= 12) score++; else suggestions.Add("Use at least 12 characters; 16 or more is better.");
        ...
        if (HasCommonPattern(secret)) { score = Math.Max(0, score - 2); suggestions.Add("Avoid common passwords, sequences, repeated characters, names, and predictable patterns."); }
        if (secret.Distinct().Count() < Math.Min(6, secret.Length)) { score = Math.Max(0, score - 1); suggestions.Add("Use more varied characters or a randomly generated passphrase."); }
    ...
    {
        if (options.Length is < 8 or > 256) throw new ArgumentOutOfRangeException(nameof(options), "Password length must be between 8 and 256.");
        ...
        if (sets.Count == 0) throw new ArgumentException("At least one character group must be selected.", nameof(options));
