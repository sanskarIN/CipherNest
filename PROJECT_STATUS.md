...
- Centralized project metadata now includes the optional development-support URL `https://buymeacoffee.com/sanskarIN`; About exposes explicit user-initiated repository/creator/support links and GitHub `.github/FUNDING.yml` points to the same support URL.
- Optional development support is documented as voluntary and does not change feature access, privacy/security treatment, support priority, licensing, or recovery behavior.
...
- The exact current policy for an external Buy Me a Coffee/funding call to action must be checked for every target store/distribution/region before packaging. If a store build cannot expose it, the in-app CTA must be omitted/disabled for that build while repository/GitHub funding metadata can remain available where permitted.
...
The ordered release/development plan is maintained in `docs/NEXT_STEPS.md`. The immediate sequence is clean build/tests/format/analyzers, platform smoke tests, real-device security-behavior validation, backup/transfer compatibility, accessibility/localization/responsive checks, performance/large-vault measurements, dependency/license/security review, signed packaging, independent security review, and only then an evidence-backed tagged release.
